...
const OurProcess = () => {
  ...
  return (
    <div className="md:p-[120px] px-[30px] py-[100px] flex flex-col items-center justify-center gap-[60px] bg-surface">
      ...
      />
      <div className="hidden md:grid grid-cols-2 grid-rows-4 w-full h-[575px] gap-y-[25px] relative overflow-x-hidden">
      ...
      <div className="md:hidden flex flex-col w-full gap-y-[25px] relative border-l border-border overflow-x-hidden md:overflow-hidden">
        {steps.map((step, index) => (
          <motion.div
            className={`col-span-2 flex items-center gap-[15px] h-full`}
            initial={{ opacity: 0, x: index % 2 === 0 ? "-100%" : "100%" }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 1, ease: "easeInOut", delay: index * 0.5 }}
            viewport={{ once: true }}
